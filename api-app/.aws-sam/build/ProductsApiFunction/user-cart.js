...
const postHandler = async (event, context) => {
    ...
    try {
        ...
        if (!isUser) {
            ...
            const scanOutput = await db_functions_1.dynamoDbClient
                .scan({
            ...
            if (scanOutput && scanOutput.Items && scanOutput.Items.length > 0) {
                for (let existingItem of scanOutput.Items) {
                    ...
                    const deleteItemOutput = await db_functions_1.dynamoDbClient
                        .deleteItem({
                        TableName: "UserCartTable",
                        Key: {
                            username: {
            ...
            if (items.length > 0) {
                for (let item of items) {
                    ...
                    const putItemOutput = await db_functions_1.dynamoDbClient
                        .putItem({
                        TableName: "UserCartTable",
                        Item: {
                            username: {
        ...
        response = {
            ...
            body: JSON.stringify({
                message: `${err.message} : an item with this id already exists`,
            }),
            headers: {
                "Access-Control-Allow-Origin": "*",
            },
    ...
    try {
        ...
        if (!isUser) {
            ...
            if (scanOutput && scanOutput.Items && scanOutput.Items.length > 0) {
                for (let existingItem of scanOutput.Items) {
                    let productId = existingItem["productId"].S;
                    let amount = existingItem["amount"].N
                        ? existingItem["amount"].N
                        : "0";
                    // let productDetails =
                    let id = productId ? productId : "0";
                    let outputProduct = await db_functions_1.dynamoDbClient
                        .scan((0, product_helper_1.getProductScanParameters)(id))
                        .promise();
